defmodule YokaiSeptetWeb.HomeLive do
  ...
  defp top_bar(assigns) do
    ...
    <header style="display: flex; align-items: center; justify-content: space-between; padding: 20px 40px; border-bottom: 1px solid var(--line);">
      <button phx-click="nav" phx-value-to="home" style="background: none; border: none; cursor: pointer; padding: 0;">
        <.logo size={40} />
      </button>
  ...
  defp logo(assigns) do
    ...
    <div style="display: flex; align-items: center; gap: 16px;">
      ...
      <div>
        <div class="kanji" style={"font-size: #{@size * 0.5}px; font-weight: 500; line-height: 1; color: var(--sumi);"}>
          妖怪七番
        </div>
  ...
  defp home(assigns) do
    modes = [
      %{id: "4p", label: "Four Players", sub: "Teams of two", kanji: "四", detail: "12 cards each · capture 4 Bosses · first to 7 points"},
      %{id: "3p", label: "Three Players", sub: "Free for all", kanji: "三", detail: "16 cards each · capture 3 Bosses · first to 7 points"},
      %{id: "2p", label: "Two Players", sub: "Pocket duel", kanji: "二", detail: "Simplified · capture 4 Bosses to win the round"}
    ]
    ...
    <div class="washi-bg" style="min-height: 100vh; display: flex; flex-direction: column;">
      ...
      <main style="flex: 1; display: grid; grid-template-columns: 1.1fr 0.9fr; gap: 80px; padding: 60px 80px; max-width: 1440px; margin: 0 auto; width: 100%;">
        <div class="slide-up">
          ...
          <h1 class="kanji" style="font-size: 88px; line-height: 0.95; margin: 0; font-weight: 500; color: var(--sumi); letter-spacing: 0.02em;">
            妖怪<br />七番
          </h1>
          <div style="display: flex; align-items: baseline; gap: 16px; margin-top: 12px; margin-bottom: 32px;">
            <span style="font-size: 28px; color: var(--shu); font-family: var(--kanji);">Yokai Septet</span>
          ...
          <div style="margin-top: 44px;">
            ...
            <div style="display: grid; gap: 12px;">
              <%= for m <- @modes do %>
                ...
                >
                  <div class="kanji" style={"font-size: 40px; color: #{if active?, do: "var(--shu)", else: "var(--sumi-mute)"}; line-height: 1; font-weight: 500;"}>{m.kanji}</div>
                  <div>
                    <div style="font-size: 18px; font-weight: 500; color: var(--sumi); margin-bottom: 2px;">{m.label}</div>
                    <div style="font-size: 13px; color: var(--sumi-mute); font-style: italic;">{m.sub} — {m.detail}</div>
                  </div>
                  <div style={
          ...
          <div style="display: flex; gap: 16px; margin-top: 40px; align-items: center;">
            <button class="btn btn-shu" style="padding: 16px 36px; font-size: 16px; letter-spacing: 0.1em;" phx-click="start">
  ...
  defp card_fan(assigns) do
    bosses =
      Enum.map(Cards.suits(), fn s ->
        %{suit: s.id, rank: List.last(s.ranks)}
      end)
    ...
    <div style="position: relative; width: 540px; height: 560px; display: flex; align-items: center; justify-content: center;">
      <svg viewBox="0 0 400 400" style="position: absolute; inset: 0; width: 100%; height: 100%; opacity: 0.7;">
        <defs>
          <filter id="ink-rough">
        ...
        <circle cx="200" cy="200" r="170" fill="none" stroke="var(--sumi)" stroke-width="3"
                stroke-dasharray="0 12 1000" stroke-linecap="round" opacity="0.18"
      ...
      <div style="position: absolute; bottom: 0; left: 0; right: 0; text-align: center;">
        ...
        <div style="margin-top: 6px; font-size: 13px; color: var(--sumi-mute); font-style: italic;">One from each suit · capture them all</div>
      </div>
    </div>
    """
  end
  ...
  defp rules(assigns) do
    sections = [
      %{
        ...
        body: "49 cards in 7 suits. Wind is the weakest, Snow is the strongest. Each suit's rank range differs — the small dots in the corner show where the card sits within its suit. The strongest card of each suit is its Boss Yokai.",
        visual: :suits
      },
      %{
        num: "2",
        title: "The A Card",
        body: "Wind's lowest card is the A. Despite its low rank, it is the most powerful card in the game — it always wins the trick when played. The player dealt the A leads the first trick.",
        visual: :a_card
      },
      %{
        num: "3",
        title: "The Pass",
        body: "After dealing, each player passes 3 cards to their teammate (or to the player on the left, in 3-player). Use this to set up your team — give your partner what they need, dump what hurts you.",
        visual: :pass
      },
      %{
        num: "4",
        title: "Tricks & Trump",
        body: "The lead player plays any card. Others must follow suit if they can. Highest card of the lead suit wins — unless trump is played, in which case the highest trump wins. The A always trumps everything.",
        visual: :trick
      },
      %{
        num: "5",
        title: "Greed is a Curse",
        body: "If you (or your team) take 7 tricks before capturing enough Bosses, you LOSE the round and your opponents win. Don't grab tricks just to grab them — the only thing that scores is capturing Bosses.",
        visual: nil
      },
      %{
        num: "6",
        title: "Round Ends",
        body: "A round ends when (a) one team has captured 4 Bosses (3 in a 3-player game), (b) one team has taken 7 tricks (greed loss), or (c) every card has been played — whoever took the last trick wins.",
        visual: nil
      },
      %{
        num: "7",
        title: "Scoring",
        body: "Only the winning team scores. Discard the Boss of the trump suit — it doesn't score. Then count the gold pip on each remaining captured Boss. Wind's Boss has 0 points; Snow's Boss is worth 3.",
        visual: :scoring
      },
      %{
        num: "8",
        title: "Victory",
        body: "First team (or player) to 7 points wins the game. A typical game runs several rounds — each round resets, but score persists.",
        visual: nil
      }
    ]
    ...
    <div class="washi-bg" style="min-height: 100vh;">
      ...
      <main style="max-width: 1100px; margin: 0 auto; padding: 60px 80px;">
        ...
        <h1 class="kanji" style="font-size: 56px; font-weight: 500; margin: 8px 0 8px; color: var(--sumi);">
          How to Play
        </h1>
  ...
  defp rules_section(assigns) do
    ...
    <div style="display: grid; grid-template-columns: 60px 1fr; gap: 20px;">
      <div style="font-size: 36px; color: var(--shu); line-height: 1; font-weight: 600; font-family: var(--sans);">{@section.num}</div>
      <div>
        <h2 style="font-size: 22px; margin: 4px 0 12px; font-weight: 500; color: var(--sumi);">{@section.title}</h2>
    ...
    <div style="display: flex; gap: 6px; flex-wrap: wrap; justify-content: center;">
      <%= for s <- @suits do %>
        <div style="display: flex; flex-direction: column; align-items: center; gap: 4px;">
          <.yokai_card suit={s.id} rank={List.last(s.ranks)} width={62} />
          <div style="font-size: 10px; font-family: var(--sans); color: var(--sumi-mute); letter-spacing: 0.2em;">{String.upcase(s.name)}</div>
        </div>
      <% end %>
    </div>
    """
  ...
  defp rule_visual(%{which: :trick} = assigns) do
    ...
    <div style="display: flex; gap: 4px; align-items: center; justify-content: center;">
      ...
      <div style="position: relative;">
        ...
        <div style="position: absolute; top: -8px; right: -8px; width: 22px; height: 22px; background: var(--gold-bright); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 11px; font-family: var(--kanji); color: var(--sumi); font-weight: 600;">勝</div>
      </div>
    </div>
    """
  end
  ...
  defp rule_visual(%{which: :scoring} = assigns) do
    rows = [
      {:wind, 6, 0}, {:earth, 7, 0}, {:mist, 8, 1}, {:river, 9, 1},
      {:forest, 10, 2}, {:flame, 11, 2}, {:snow, 12, 3}
    ]
      ...
      <%= for {s, r, p} <- @rows do %>
        <div style="display: flex; flex-direction: column; align-items: center; gap: 4px;">
          ...
          <div style="display: flex; gap: 2px;">
            <%= if p > 0 do %>
              <%= for i <- 1..p do %>
                <div style="width: 6px; height: 6px; border-radius: 50%; background: var(--gold-bright);" data-i={i}></div>
              <% end %>
            <% else %>
