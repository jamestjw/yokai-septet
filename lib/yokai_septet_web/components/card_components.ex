defmodule YokaiSeptetWeb.CardComponents do
      ...
      <%= case @suit do %>
        ...
        <% :earth -> %>
          ...
          >
            ...
            <path
              d="M14 24 L18 20 L20 22"
              fill="var(--washi-warm)"
              stroke="var(--washi-warm)"
              stroke-width={0.6 * @sw}
            />
          </g>
        <% :mist -> %>
          <g>
            <path
              d="M20 6 C14 8 10 14 12 20 C14 24 18 22 18 26 C18 30 14 30 14 34 L20 32 L26 34 C26 30 22 30 22 26 C22 22 26 24 28 20 C30 14 26 8 20 6 Z"
              fill="currentColor"
              stroke="currentColor"
              stroke-width={1.2 * @sw}
            />
        ...
        <% :river -> %>
          <g>
            <path
              d="M20 6 C14 14 10 22 14 30 C18 36 22 36 26 30 C30 22 26 14 20 6 Z"
              fill="currentColor"
              stroke="currentColor"
              stroke-width={1.2 * @sw}
            />
        ...
        <% :flame -> %>
          <g>
            <path
              d="M20 6 C16 12 12 14 12 22 C12 30 16 34 20 34 C24 34 28 30 28 22 C28 14 24 12 20 6 Z"
              fill="currentColor"
              stroke="currentColor"
              stroke-width={1.2 * @sw}
            />
            <path
              d="M20 14 C18 18 17 22 19 28"
              fill="none"
              stroke="var(--washi-warm)"
              stroke-width={0.8 * @sw}
              stroke-linecap="round"
  ...
  def suit_emblem(assigns) do
    ...
    <span
      class="kanji"
      style={"display: inline-flex; align-items: center; justify-content: center; color: #{@color}; font-size: #{@size}px; font-weight: 600; line-height: 1;"}
    >
      {@kanji}
    </span>
    """
  ...
  def point_pips(assigns) do
    ...
    <%= if @solid > 0 or @outline > 0 do %>
      <div style="display: flex; gap: 2px; align-items: center;">
        <%= for i <- 1..@solid//1 do %>
          <div
            style={"width: #{@size}px; height: #{@size}px; background: var(--gold-bright); border-radius: 50%; box-shadow: 0 0 0 1px rgba(0,0,0,0.2);"}
            data-i={i}
          >
          </div>
        <% end %>
        <%= for i <- 1..@outline//1 do %>
          <div
            style={"width: #{@size}px; height: #{@size}px; border: 1.5px solid var(--gold-bright); border-radius: 50%;"}
            data-i={i}
          >
          </div>
        <% end %>
  ...
  defp yokai_card_face(assigns) do
    ...
    >
      ...
      <%= if @is_boss do %>
        <div style="position: absolute; inset: 3px; border: 1.5px solid var(--gold-bright); border-radius: 4px; pointer-events: none; box-shadow: inset 0 0 12px rgba(212, 175, 55, 0.25);">
        </div>
      <% end %>

      <%= if @is_a do %>
        <div style="position: absolute; inset: 3px; border: 2px solid var(--shu); border-radius: 4px; pointer-events: none; box-shadow: inset 0 0 14px rgba(200, 72, 60, 0.3);">
        </div>
      <% end %>
      ...
      <div style={"position: absolute; top: #{@width * 0.06}px; left: #{@width * 0.08}px; display: flex; align-items: flex-start; gap: #{@width * 0.04}px;"}>
        <div>
          ...
          <div
            class="kanji"
            style={"font-size: #{@corner_size * 0.55}px; margin-top: 2px; opacity: 0.8; line-height: 1; font-weight: 500;"}
          >
            {@suit_data.kanji}
          </div>
        </div>
      ...
      <%= if @is_boss and (@suit_data.boss_pts4 > 0 or @suit_data.boss_pts3_extra > 0) do %>
        <div style={"position: absolute; left: 50%; bottom: #{@width * 0.34}px; transform: translateX(-50%); display: flex; flex-direction: column; align-items: center; gap: 3px;"}>
          <.point_pips
            solid={@suit_data.boss_pts4}
            outline={@suit_data.boss_pts3_extra}
            size={@width * 0.09}
          />
        </div>
      <% end %>
      ...
      <%= if @is_boss do %>
        <div
          class="kanji"
          style={"position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%); font-size: #{@width * 0.85}px; color: #{@suit_data.color}; opacity: 0.07; font-weight: 700; pointer-events: none; line-height: 1;"}
        >
          七
        </div>
      <% end %>
      ...
      <%= if @is_a do %>
        <div style={"position: absolute; top: 50%; left: 0; right: 0; transform: translateY(-50%); text-align: center; font-family: var(--serif); font-size: #{@width * 0.36}px; color: var(--shu); font-weight: 700; letter-spacing: 0.02em; text-shadow: 0 0 16px rgba(200, 72, 60, 0.35); pointer-events: none;"}>
          A
        </div>
      <% end %>
    </div>
